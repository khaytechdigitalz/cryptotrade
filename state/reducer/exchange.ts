...
export const exchangeSlice = createSlice({
  ...
  reducers: {
    ...
    setStopLimitOrders: (state: any, action: PayloadAction<Array<any>>) => {
      state.stopLimitOrders = action.payload;
    },
    setValidationRulesForSpot: (
      state: any,
      action: PayloadAction<Array<any>>
    ) => {
      state.validationRulesForSpot = action.payload;
    },
    setTotalData: (state: any, action: any) => {
      state.dashboard.order_data.total = {
        ...state.dashboard.order_data.total,
        trade_wallet: {
          ...state?.dashboard?.order_data?.total?.trade_wallet,
          ...action.payload?.trade_wallet,
        },
        base_wallet: {
          ...state.dashboard.order_data?.total?.base_wallet,
          ...action.payload?.base_wallet,
        },
      };
    },
    setOnOrderData: (state: any, action: any) => {
      state.dashboard.order_data.on_order = {
        ...state.dashboard.order_data.on_order,
        ...action.payload,
      };
    },
