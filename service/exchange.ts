...
// get tolerence validation for limit

export const getValidationRulesForSpot = async (
  base_coin_id: string,
  trade_coin_id: string
) => {
  const { data } = await request.get(
    `/limit-order-tolerence?base_coin_id=${base_coin_id}&trade_coin_id=${trade_coin_id}`
  );
  return data;
};
